...
// CODE HERE
const evenNumbers = mixedNumbers.filter(element => element % 2 === 0) //.filter will create anothe array on this variable.
...
/*
  ...
  function(runningTotal, curElement, curIndex, wholeArray){} Function Form  /accumulator is the wrong word for this from the slides. runningtotal is so much better. but more specifically, runningelement would do it
  (runningTotal, curElement, curIndex, wholeArray)=>{} Arrow Form
*/

// CODE HERE
 const totalPopulation = populations.reduce((acc, curr) => acc + curr) //taking anonymous callback function as the running total
...
const totalsaftertax = orders.map(element => element.price * (element.tax+1))
